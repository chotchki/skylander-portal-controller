...
- [x] 4.19.23 ~~*[feature]* **Server-error launcher state for "server failed to start" (user-flagged 2026-04-19).**~~ **Done 2026-04-19** — `LauncherScreen::ServerError { message }` variant added; the tokio thread reports each startup failure (driver build, db path resolve, profile-store open, listener bind, axum-serve exit) via a `report_fatal` helper that sets the screen + logs. Replaced the old `expect("bind")` panic so a port-in-use no longer takes down the whole process. `ui/server_error.rs` renders a "SERVER FAILED TO START" surface with the diagnostic message + Exit-to-Desktop button. Recovery is exit-only (not RESTART) — startup errors are infrastructure problems the launcher can't fix from inside the process.
