...
- [ ] 6.2.7 **Parse-driven figure discovery (identity-only, no decryption required).** **Blocked on 6.5.0 spike outcome (2026-04-22).** If the spike confirms ACS122U scanning is viable, this item is subsumed by 6.5.5 — scanning makes parse-driven identity the canonical path, and a folder-walk refactor becomes vestigial. If the spike reveals blockers (Windows driver pain, latency too high, Key-A derivation infeasible), revive this as a dual-key pack-walk refactor: keep the SHA-based `FigureId` as the `data/figures.json` join key, teach `crates/indexer/` to resolve `(tag_figure_id, variant)` from block 0, fall back to folder-walk when parse fails. Original context: folder-walk is brittle against real-world dumps — `./dev-data/validation-figures/` has filenames like `Bad juju(villian.sky`, `grave clobber (villan.sky`, `Wolfgang (vilian).sky`, and a `traps/` subdir that per our range-table classifies contents as `Standard` (likely captured-villain figure_id). Risk to watch either way: trap crystals apparently store a captured-villain figure_id rather than a distinct "trap figure" ID (see validator output for `traps/Air Trap.sky` at fid=0x0000D4), so identity lookup alone won't distinguish "Air Trap" from "level-1 captured villain with the same ID" — disambiguation rule needs to consider additional bytes. Pin once 6.5.0 resolves the scope.
...
- [~] 6.5 **NFC-scan import via ACS122U reader.** Plus-button in the toy-box view; one-way import (read-only, no write-back per 2026-04-22 scope call). Scanned figures join the shared toy-box library — available to any profile, working copies kept per-profile (same semantics as pack-derived figures in CLAUDE.md). Primary goals: (a) discourage piracy by making scan-and-own the easiest onboarding path; (b) decouple figure discovery from user-maintained file structure; (c) accommodate users who have physical figures but no existing .sky backup. Child items gate on 6.5.0's spike outcome.
  - [ ] 6.5.0 **SPIKE: prove ACS122U + Key-A + full 1K read end-to-end.** One-evening probe *before* designing the real flow. Deliverable: a minimal CLI under `tools/nfc-dump/` that waits for a card, computes Key A via CRC48-ECMA-182 on block-0/1 (blog Appendix A), authenticates each sector, reads all 64 blocks (1024 bytes), writes `<uid>.sky`. Success = piping that output through `cargo run --example validate_samples -p skylander-sky-parser -- <dir>` yields the same decoded stats as the same physical figure's existing sample file in `./dev-data/validation-figures/` (ground-truth pair: pick a figure Chris has both physically and as a dump, e.g. Tree Rex). Questions the spike must answer:
    - Rust crate choice: `pcsc` (PC/SC, cross-platform, Windows-native) vs `nfc1` (libnfc bindings, may need libnfc install). Default to `pcsc` unless it can't auth Mifare Classic.
    - Key A derivation correctness: validate against a dump we already decode cleanly — if the derived key successfully authenticates, reads match, and parser sees the same figure_id/level/XP, the algorithm is confirmed.
    - Latency: seconds from tap → full 1K dumped. Drives UX: < 2s feels like "tap and done"; > 5s means the import flow needs a progress bar + batch-unfriendly scoping.
    - Windows driver story: does ACS122U work with the inbox CCID driver under Steam Big Picture boot, or does it need ACS's installer? Packaging implication for 7.1 (single-exe goal).
    - Failure modes: what happens on card-too-fast, wrong card type (MIFARE Ultralight posers), UID clashes.
  - **Explicitly out of scope for the spike:** server integration, phone UI, storage-layout decision, onboarding rework. CLI + a .sky file + a latency number + a short `docs/research/acs122u-spike.md` writeup is the entire deliverable. If any of the four questions above fail, write up the failure and revert to 6.2.7's dual-key fallback plan.
