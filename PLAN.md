...
- [ ] 4.18.1d **iOS dark-band at bottom persists despite three fix attempts.** Chris flagged 2026-04-21. Tried (1) `height: 100dvh` on body + `overscroll-behavior: none`, (2) `background-attachment: fixed` on body, (3) `body::before { position: fixed; inset: 0 }` gradient layer. Third attempt matches the `body::after` starfield pattern that already works on iOS, but a thin dark band still renders at the bottom of the visible viewport on Chris's iPhone. Headless Chrome can't reproduce (no address bar transition; visible area equals `100dvh` exactly) — see `crates/e2e-tests/tests/visual_debug_background.rs` harness. **Candidates to investigate**: (a) iOS home-indicator strip rendering over the PWA at a depth our pseudo can't cover, (b) `env(safe-area-inset-bottom)` padding on `.app` stealing height from the visible background area, (c) iOS compositor layer caching the pseudo against an outdated viewport rect when the URL bar retracts. Low priority until we have a decisive repro (physical device + Web Inspector). **Update 2026-04-21**: `tools/ios-inspect/` (PLAN 4.21) now provides a sim-side probe loop — re-attempt tracked as 4.21.4, caveat that sim reports `safe-area-inset-bottom = 0` so candidate (b) needs PWA-standalone mode or real-device validation. **Diagnosis complete (4.21.4)**: dark band is `html`'s flat `#020818` exposed below the fixed `body::before` pseudo when iOS retracts the URL bar — color-seams against the gradient's lighter bottom-center radial highlight (~`#0e2464`). Fix tracked as 4.18.1e.
- [ ] 4.18.1e **Fix dark band by moving gradient off `body::before` onto `html`.** In `phone/assets/app.css`: (1) replace `html { background: var(--sf-3); }` with the full gradient stack currently on `body::before` (the 3-layer radial+radial+linear), (2) delete the background from `body::before` and drop the pseudo entirely if no other rule targets it (the only job it was doing was carrying the gradient layer). `html`'s bg paints across the full canvas including the URL-bar-retraction region, so no seam is possible. Kaos skin's `body.skin-kaos` override currently paints onto `body` — either mirror to `html.skin-kaos` via class-on-html (set in `phone/src/pwa.rs` alongside body class), or change the skin selector to target `html:has(body.skin-kaos)` (modern iOS Safari supports `:has`). Validate with `ios-inspect` after: `ios-inspect eval 'window.scrollTo(0,100); getComputedStyle(document.documentElement).backgroundImage.includes("radial-gradient")'` should be truthy; on real device, scroll to trigger URL-bar retract and confirm no band. Keep 4.18.1d open until a real-device run confirms the fix (sim can't reproduce).
