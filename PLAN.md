...
- [ ] 6.2.9 **Pin Vehicle + CYOS `figure_id` ranges against real dumps.** 6.2.2 left these ranges commented out because the community-sourced values (Vehicles 0x2BC..=0x2ED, CYOS 0x320..=0x383) missed every real sample. Observed vehicle-looking figure_ids cluster near 0x0C9x..=0x0CAx but overlap with SuperChargers characters. **First live-scan CYOS data point (2026-04-22)**: a creation crystal scanned via the 6.5.0 spike reported fid=**0x0002AD** variant=0x520F — currently misclassified as `Standard`, which is why its nickname decoded as UTF-16 mojibake (`䕄䙌塏`: two ASCII bytes jammed into each UTF-16 codepoint). CC user-chosen names live at a different offset + encoding than Standard figures, so correct `FigureKind::Cyos` classification is the gate on readable nicknames. Need larger CYOS sample set before drawing a safe range — one fid doesn't make a contour. Still blocks 6.2.4 / 6.2.5. Deliverables: (a) Chris drops SuperChargers vehicle dumps + ≥3 more CYOS dumps via 6.5.0's scan tool into `./dev-data/validation-figures/`; (b) extend `FigureKind` range table with new entries + a test per variant using real observed fids; (c) verify nickname decodes cleanly for CC once 6.2.5's CYOS parser lands.
...
- [~] 6.5 **NFC-scan import via ACS122U reader.** Plus-button in the toy-box view; one-way import (read-only, no write-back per 2026-04-22 scope call). Scanned figures join the shared toy-box library — available to any profile, working copies kept per-profile (same semantics as pack-derived figures in CLAUDE.md). Primary goals: (a) discourage piracy by making scan-and-own the easiest onboarding path; (b) decouple figure discovery from user-maintained file structure; (c) accommodate users who have physical figures but no existing .sky backup. Child items gate on 6.5.0's spike outcome.
  ...
  - [ ] **6.5.2 follow-up** (post-live-test): UI validation on the real server with `--features nfc-import` + ACR122U + a phone — tap figure, confirm overlay flips to Success; tap figure with overlay closed, confirm toast fires. Timeout path is client-only and testable without hardware (tap +, wait 30s).
