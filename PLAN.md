...
- [ ] **3.7.8 Game-list hardening.** Source `/api/games` from the running RPCS3 (UIA walk of `game_list_table` or `rpcs3.exe` CLI dump if one exists) rather than `games.yml`. Current failure mode: phone picks a serial `games.yml` advertises but RPCS3's library doesn't have. Blocked on 4.15.15 egui-shell-at-startup direction.
...
- [ ] 4.20.1 Extract `<BlueCard>` / `<ActionButton>` (§6.6). Currently inlined in `modals.rs` (menu overlay actions), `figure_detail.rs` (action row), resume/reset confirms. Props: `variant: default|danger`, optional `leading_icon`, title, description, `on_click`, optional `hold_duration`. Absorbs the hold-to-activate `pointerdown`/`pointerup` glue so SHUT DOWN + CHOOSE ANOTHER GAME + HOLD TO RESET stop each reimplementing it.
- [ ] 4.20.2 Extract `<ToyBoxLid>` / `<ToyBoxInterior>` (§6.7). Currently inlined in `browser.rs` (already has `BoxState::{Closed, Compact, Expanded}` + pointer-capture drag handlers — split rather than rewrite). Keeps the gesture logic with the component.
- [ ] 4.20.3 Extract `<KebabMenuOverlay>` (§6.8). Currently in `modals.rs`. Props: open signal, actions, join URL, profile. Absorbs the QR-wiring slot for 3.10.8 (Phase 3 carryover).
- [ ] 4.20.4 Relocate `<AppHeader>` (§6.9) from `screens/header.rs` to `components/header.rs` — thin move so composition points outside the main screen stack can reuse it.
...
- [ ] 4.20.5a Extract `<ConnectionLostOverlay>` (4.18.21), `<GameCrashScreen>` (4.15.14), `<PwaHint>` (4.18.1b) into `components/` — OR explicitly label them "screen-level, not component-extracted" in the design doc. Decide per-component during extraction; whichever way, the doc should match the code.

**Token standardization (B).**

- [ ] 4.20.6 **Declare typography scale in `:root`.** `--t-display-hero/lg/md/sm/xs`, `--t-body`, `--t-body-italic`, `--t-body-sm`. Values should match what the shipped UI actually renders today (spot-check against `ui_style_example.png` at viewing distance — don't blindly copy §2's original numbers if the shipped code has already tuned them). Update `design_language.md` §2 if the shipped values differ from the doc.
...
- [ ] 4.20.8 **Declare motion tokens in `:root`.** `--dur-impact: 600ms`, `--dur-shudder: 400ms`, `--dur-sky-drift: 140s`, `--dur-hold-confirm: 1200ms` (the last is currently referenced with inline fallback only). Plus any new ones 4.20.9 discovers.
