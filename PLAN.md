...
- [x] **3.16.5 First-launch wizard live-desktop verify.** Closed 2026-04-23 alongside 6.5.4 live verification — release-mode wizard clicks through end-to-end, config.json persists, server boots from the written config. Reader-detection side specifically stays gated on physical console (see 6.5.4 RDP note).
...
- [ ] 4.18.29 *[bug]* **Reset browser search on game change.** The `search` signal in `phone/src/lib.rs` persists across `current_game` transitions, so a query typed while Giants was booted keeps filtering the library after switching to Trap Team. Likely also applies to `element_filter` / `game_filter` / `category_filter`, though those are less disruptive since they're visibly toggled. Fix: wire an `Effect` that clears all four signals when `current_game` changes (skip the initial `None → Some(_)` run so search doesn't reset on first game boot). Surfaced 2026-04-23 during 6.6 live verification.
...
- [x] 6.2.7 **Parse-driven figure discovery (identity-only, no decryption required).** Closed 2026-04-22 — superseded by 6.5.5. 6.5.0 spike proved ACS122U scanning viable (~1.4s/figure, CRC-valid dumps parsing correctly); folder-walk discovery is now vestigial. Original context preserved for 6.5.5: `./dev-data/validation-figures/` has filenames like `Bad juju(villian.sky`, `grave clobber (villan.sky`, `Wolfgang (vilian).sky`, and a `traps/` subdir that our range-table classifies as `Standard` because trap tags hold a captured-villain figure_id rather than a distinct "trap figure" ID (see validator output for `traps/Air Trap.sky` at fid=0x0000D4). Disambiguation rule — "Air Trap" vs "level-1 captured villain with the same ID" — needs to consider additional bytes; 6.5.5 owns this as it migrates `data/figures.json` off SHA-based keying.
...
- [ ] 6.2.9 **Pin Vehicle + CYOS `figure_id` ranges against real dumps.** 6.2.2 left these ranges commented out because the community-sourced values (Vehicles 0x2BC..=0x2ED, CYOS 0x320..=0x383) missed every real sample. Observed vehicle-looking figure_ids cluster near 0x0C9x..=0x0CAx but overlap with SuperChargers characters. **First live-scan CYOS data point (2026-04-22)**: a creation crystal scanned via the 6.5.0 spike reported fid=**0x0002AD** variant=0x520F — still classified as `Standard` until ranges land here. **Nickname decode was solved out-of-band 2026-04-22** via an auto-detect heuristic in `sky-parser::decode_nickname` (UTF-16 LE if every odd byte in the populated run is `0x00`, else pack-ASCII). DELFOX now reads correctly on both the stats endpoint and the scan toast. Still want FigureKind::Cyos classification so the UI can branch detail screens (6.2.5) and so the join at 6.5.5 knows to treat these as CYOS. Need larger CYOS sample set before drawing a safe range — one fid doesn't make a contour. Still blocks 6.2.4 / 6.2.5. Deliverables: (a) Chris drops SuperChargers vehicle dumps + ≥3 more CYOS dumps via 6.5.0's scan tool into `./dev-data/validation-figures/`; (b) extend `FigureKind` range table with new entries + a test per variant using real observed fids; (c) smoke-test `decode_nickname` against the new CYOS samples to make sure the auto-detect holds.
...
- [~] 6.5 **NFC-scan import via ACS122U reader.** Plus-button in the toy-box view; one-way import (read-only, no write-back per 2026-04-22 scope call). Scanned figures join the shared toy-box library — available to any profile, working copies kept per-profile (same semantics as pack-derived figures in CLAUDE.md). Primary goals: (a) discourage piracy by making scan-and-own the easiest onboarding path; (b) decouple figure discovery from user-maintained file structure; (c) accommodate users who have physical figures but no existing .sky backup. Child items gate on 6.5.0's spike outcome.
  - [x] 6.5.0 **SPIKE: prove ACS122U + Key-A + full 1K read end-to-end.** Done 2026-04-22. CLI at `tools/nfc-dump/` dumps 1024 bytes end-to-end in **~1.4 seconds** (well under the 2s "tap and done" bar). Two live figures scanned + parsed CRC-valid: creation crystal (uid=7FC1ADA3, fid=0x0002AD) and Snap Shot Trap Master (uid=2F685EC4, fid=0x01CE, lvl 15, 65258 gold, default nickname decoded). Key-A CRC48 derivation matches blog Appendix A. Transport: **`pcsc` crate in `ShareMode::Direct` + `SCardControl(IOCTL_CCID_ESCAPE=3500)`**. Pitfalls logged so they don't bite future bring-up:
    - **macOS needs the ACS Unified CCID Driver installed** (https://www.acs.com.hk/en/driver/3/acr122u-nfc-reader/). Apple's inbox CCID driver silently rejects `SCardControl` escape (`InvalidParameter`), blocking the PN532-direct path entirely. Post-install the reader re-enumerates as "ACS ACR122U" (short name, vs inbox "ACS ACR122U PICC Interface").
    - **IOCTL code matters**: the ACS driver on macOS mirrors the Windows value `SCARD_CTL_CODE(3500)` not the pcsc-lite default `SCARD_CTL_CODE(1)`. Using 1 returns `NoTransaction` even with the ACS driver. Spike code hard-codes 3500 — will need to smoke-test on Linux (expect same ACS value) and Windows (native value, should match).
    - **pcsc crate panics on `Direct + ANY`**: macOS returns T0|T1 together as active protocol; `Protocol::from_raw` panics. Request `Protocols::T1` specifically. Documented in `connect_direct()`.
    - **Mifare Classic 1K never activates under PC/SC** (ISO14443-3 only, no native ATS) — `ShareMode::Shared` waits forever. `ShareMode::Direct` attaches to the reader itself, no card required, which is what lets us reach the PN532 at all.
    - **Write-path pivots attempted and abandoned**: raw USB via `rusb` (macOS kernel IOCCIDDriver holds the interface exclusively — libusb can't claim without DriverKit entitlement), and HID-direct to a real Skylanders Portal of Power (hidapi `write` is silently dropped by the macOS HID layer — the SkylandersToolkit reference implementation uses `DeviceIoControl(0xB0195)` on Windows specifically to bypass the HID stack; no user-space equivalent on macOS). Both dead ends on macOS; PC/SC + ACS driver is the only viable path.
  - Resolved follow-ups (moved out of this item): 6.2.7 is officially superseded by 6.5.5 (scanning proven, folder-walk discovery is vestigial); 6.2.9 gains concrete CYOS data point fid=0x0002AD (creation crystal, currently misclassified as `Standard`).
  - [x] 6.5.1 **Scanner worker + event stream.** Done 2026-04-22. Spike code lifted out of `tools/nfc-dump/` into new lib crate `crates/nfc-reader/` (lib + thin `nfc-dump` bin wrapper). Server gains optional dep on the new crate behind the `nfc-import` feature (off by default — base build has zero pcsc linkage). New `crates/server/src/nfc.rs` spawns `run_scanner_worker` on a dedicated OS thread (beat tokio's `spawn_blocking` pool for a never-ending poll loop). Worker loops: `open_reader → poll → dump → persist → broadcast`; handles reader unplug/replug via `RECONNECT_BACKOFF` + reopen; 5s uid-level dedupe window stops "figure left on pad" from spamming events. Raw bytes stay server-side (written to `<data_root>/scanned/<uid>.sky`); broadcast event carries only identity: `Event::FigureScanned { uid, figure_id, variant }`. Live-verified post-refactor: Snap Shot still dumps CRC-valid in ~1.4s through the extracted lib. 149 workspace tests pass.
    - Departed from the original plan's `FigureScanned(bytes, …)` shape — 1 KB × N subscribers on the broadcast channel was wasteful, and the phone doesn't need raw bytes to refresh its library view. Byte handling is the server-side worker's job; phones just learn "a new figure appeared" and can fetch whatever they need through the library API.
    - Intentionally deferred: HTTP endpoints (6.5.2 owns them), real storage-root resolution (6.5.3 formalizes `scanned/` location), onboarding reader-detection (6.5.4).
  - [~] 6.5.2 **Plus-button scan-to-import flow in toy box.** Landed UI + WS wiring 2026-04-22 per **option C** (phased): plus-button → scan overlay → confirmation, no library-side dedup/unknown-figure logic until 6.5.5 rekeys `data/figures.json`. Done pieces:
    - Mock at `docs/aesthetic/mocks/scan_import.html` (iterated on real iPhone; scan-center uses the portal empty-slot's filled-gold donut + dark-blue plate construction so it reads as "just another slot").
    - `Event::FigureScanned` carries a parser-derived `display_name` so the phone can show "Added: SNAP SHOT" without a second RTT.
    - New Leptos component `phone/src/components/scan_overlay.rs` with Closed/Prompt/Success/Timeout states, owns its own 30s timeout timer (cancels on state change).
    - Plus-card (`fig-card-p4 scan-new`) pinned at figure-grid position 0; on click sets `ScanOverlayState::Prompt`.
    - WS handler: Prompt + `FigureScanned` → Success (flip); Closed + `FigureScanned` → passive success toast so ambient scans (figure placed without the overlay open) don't vanish silently.
    - CSS mirrors the mock 1:1 in `phone/assets/app.css`.
  - Explicitly deferred to 6.5.5 (all need the tag-id → library-entry join): real "this one's already in your collection" dedup toast, unknown-figure error branch, merging scanned figures into the main figure grid alongside pack-derived ones. Until then: scanned figures are written to `<data_root>/scanned/<uid>.sky` but don't render in the grid.
  - [x] **6.5.2 follow-up** (live test 2026-04-22). Tap-once-with-overlay confirmed Success copy. Tap-with-overlay-closed confirmed toast. Three follow-ons from the session all landed:
    - **Edge detection** in the scanner worker — figures left on the pad no longer re-broadcast every 5–6s. Lift-then-tap is the explicit "scan again" gesture.
    - **is_duplicate flag** through `Event::FigureScanned` — phone branches copy between "Added / Scanned" (new) and "Already in your collection / Already scanned" (dupe); toast level flips Success → Info so the colour code also moves.
    - **Toast accessibility**: added a per-level glyph (`✓`, `ℹ`, `⚠`, `✕`) beside the left-strip colour, so the status is redundantly encoded for colour-blind users. Kept as semantic text (not aria-hidden) so screen readers narrate it.
    - **Nickname auto-detect** in sky-parser — CC's "DELFOX" now decodes readably (was mojibake). See 6.2.9 note.
    - **Dev DATA_ROOT fix**: added `DATA_ROOT=./dev-data` to `.env.dev` so the scanner's output dir lives next to the profile db instead of in a stray `./data/scanned/` tree.
    - Timeout path not live-tested — will watch for it during in-game tests.
  - [x] 6.5.3 **Shared-pack storage layout.** Closed 2026-04-22 with option (b) + a scope trim. Layout locked in as `<data_root>/scanned/<uid>.sky` (dev: `./dev-data/scanned/`, release: `%APPDATA%\skylander-portal-controller\scanned\`), documented in CLAUDE.md. Departed from the plan's original `<figure_id>-<variant>.sky` naming — **uid-keyed wins** because it preserves per-physical-tag state. Two copies of Spyro in the same household stay distinct and keep independent gold/level state; a single canonical name would've conflated them (and the first would lose its history on the second's scan). Canonical identity dedup happens at the library-index layer (one card per `(figure_id, variant)`, implemented in 6.5.5) rather than at the file-storage layer. Working-copy semantics for scanned figures — one copy per `(profile, uid)` vs per canonical identity — also deferred to 6.5.5 since they're entangled with the rekey.
  - [x] 6.5.4 **First-launch onboarding — reader-detection + optional pack.** Landed 2026-04-22 per option B1 (ship-with-known-gap: scanned figures are invisible in the library view until 6.5.5 teaches the indexer to walk the scanned dir). Live-verified on HTPC 2026-04-23 (release build `--no-default-features --features nfc-import` clicks through Welcome → Step 1 → Step 2 with default path-guess hits; config.json written to `%APPDATA%\skylander-portal-controller\`; server boots from it). Changes:
    - New `skylander_nfc_reader::probe_reader()` — fast, side-effect-free PC/SC enumeration returning true iff an ACR122U-named reader is present. No connection, no card activation; safe to call at wizard mount.
    - Wizard Welcome page surfaces the probe result: green "✓ reader detected → you can scan figures" or grey "ℹ no reader → plug one in and re-run setup if you want scan support later".
    - Wizard firmware-pack step re-titled "Step 2 of 2 — Firmware pack (optional)". Empty input is explicitly valid (`pack_valid()` returns `Ok(())`); the Finish button label flips to "Finish (skip pack)" when blank. Copy on the step branches on `reader_present` so the pack is framed as optional-for-reader-users and strongly-recommended-otherwise.
    - `cfg.firmware_pack_root` is now a `PathBuf` that can be empty (`PathBuf::new()`); `FIRMWARE_PACK_ROOT=` / missing env maps to empty. `main.rs` guards the indexer scan — empty root logs "no firmware pack configured" and starts with an empty library instead of walking.
    - Fixed two pre-existing release-mode build errors in `config::load` (missing `anyhow::Context` import, untyped `Vec`) while I was in there — `cargo build --no-default-features --features nfc-import` compiles again.
    - Reader-absent + pack-absent is a valid boot state, matching Chris's stated end-state: Imaginators' "instant Skylander" creation flow doesn't need a pre-existing collection. Library stays empty; user can add a pack or plug in a reader later (via a re-run of setup).
  - Gap until 6.5.5: a pack-skipped user scans figures and they go to disk, but the library view stays empty (no phone UI for the scanned dir). Toast feedback still fires on every scan so the user knows it's working; the actual "added to library" rendering lands in 6.5.5.
  - **Theming polish landed with live verification** (2026-04-23). Original wizard shipped in plain egui default (white background, 720×480 window) which read as jarringly off-brand at 86" TV scale. Reskinned to match the launcher: `palette::apply` + `fonts::register` called from the CreationContext so starfield-blue base + Titan One display face carry over; `ViewportBuilder::with_fullscreen(true)`; `vortex::paint_sky_background` + `paint_starfield` as ambient backdrop; per-page renderers with gold-outlined inputs, solid-gold primary buttons, ghost outlined secondaries, em-dash-free ASCII-only copy so Titan One's limited glyph set doesn't tofu. Esc → cancel since fullscreen hides the title-bar X. Closes the parent 6.5.4.
  - **RDP gotcha discovered during validation** (saved to auto-memory `project_rdp_breaks_smartcard.md`): `SCardEstablishContext` inside an RDP session resolves to the *client's* smart card manager, not the HTPC's. `probe_reader()` correctly returns false in that context — it's not a bug, it's environmental. Full reader-path smoke (green ✓ Welcome state → actual scanner worker activation) still needs a true physical-console run; parked for the next hands-on-HTPC session.
  - [x] 6.5.5a **Scanned-figure indexer + pack-wins merge.** Done 2026-04-23. Shipped the phased MVP — scans now appear in the library, dedup is library-aware, and the pack-vs-scan collision behaves per the "pack is a fresh master" rule with a nicknames-promote escape-hatch for user customization.
    - `skylander-indexer::scan_runtime(&data_root.join("scanned"))` walks the scanned dir, parses each `<uid>.sky` for tag identity, emits `Figure` entries with `id="scan:<uid>"`, kind-based `canonical_name` ("Creation Crystal" / "Trap" / "Figure 0xNNN"), and the parsed nickname as `variant_tag`. `GameOfOrigin::Unknown` added so the compat-matrix can classify these correctly (6.5.5b will fill in the real origin).
    - **Pack wins on collision**, but with nickname promotion: if a scan matches a pack entry AND the pack's `variant_tag == "base"` AND the scan has a non-empty nickname, we overwrite the pack card's `variant_tag` with the scan's nickname. Keeps pack identity ("CRYSTAL - FIRE Reactor") while surfacing the user's customization ("DELFOX"). Working-copy forks still come from the pack master (fresh state).
    - **Variant identity mask.** `sky-parser::VARIANT_IDENTITY_MASK = 0x0BFF` — mask the year_code (bits 12..15) and is_in_game_variant (bit 10) off raw variants before keying the dedup map. Pack masters store `variant=0x0000`; physical tags carry the generation they were last played in. Raw matching would miss a live scan of a pack-owned figure; masked matching collapses them. Discovered during live test when DELFOX's scan didn't dedup against the pack's Fire Reactor master.
