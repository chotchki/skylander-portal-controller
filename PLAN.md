...
- [x] 4.19.2a ~~*[feature]* **No "Startup" surface.**~~ **Done 2026-04-19** — `crates/server/src/ui/launch_phase.rs` introduces `LaunchPhase::{Startup,Transitioning,AwaitingConnect}` derived from `time_since_mount` + activity. Startup paints starfield + a bold "STARTING" Titan-One title (embossed via stacked `Painter::text` shadow layers + 8-step radial halo per `paint_heraldic_title` — recreates the spec's CSS text-shadow stack); Transitioning ramps `iris_radius` 0→1.2 over 0.6s while the QR/status snap in; AwaitingConnect parks at the steady main render. Activity (rpcs3_running OR clients>0) short-circuits the reveal so a server restart mid-session doesn't replay the intro. 8 unit tests cover the phase math + monotonic iris + content-visibility contracts. Also added `vortex::paint_starfield` (~120 deterministic stars, three colour tints, slow per-star twinkle) — long-standing gap, the launcher now actually has a starry sky to be calm against. `palette::apply` now also pins `ThemePreference::Dark` first, fixing a Windows-11-light-mode bug where Visuals were getting clobbered back to cream after our set_visuals call.
...
- [ ] 4.19.5 *[feature]* **Vortex shader: cheap polar-mesh sin/cos → ship the real simplex FBM (user-decided 2026-04-19: "the shaders look horrible").** Real-distance sanity check came back: cheap approximation reads as banded sin/cos rather than organic clouds. Ship Path A from spec §3.2: 5-octave simplex FBM via `egui_wgpu` custom paint callback. The mock at `tv_launcher_v3.html:751-816` has the full WGSL-translatable GLSL fragment shader. Lock with a frame-render snapshot test or at minimum a "vortex paints non-trivially" smoke test on the egui_wgpu callback path.
