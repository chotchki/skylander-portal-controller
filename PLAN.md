...
- [x] 4.15.6 QR card-flip on max-players (Y-axis rotate, back face shows "MAXIMUM PLAYERS REACHED"). *Landed: `LauncherStatus.session_slots_full: bool` threaded through `AppState::publish_session_snapshot` (called after every `SessionRegistry::register`/`remove`/`set_profile` in `http.rs`). `ui/main_screen.rs::qr_card_flip` uses `ctx.animate_bool_with_time(id, flipped, FLIP_DURATION=0.5)` to drive a tent-wave horizontal scale `|2·p−1|` with content-swap at `progress > 0.5`. Egui-native, no custom matrix math — the Y-axis rotation reads identically to an X-scale in 2D. The back face renders `MAXIMUM / PLAYERS / REACHED` across three lines of Titan One `GOLD_BRIGHT` on an `SF_1` plate with the same `GOLD` bezel as the front.*
- [x] 4.15.7 Player-orbit indicators around the QR (gold-bezeled circles with profile color + initial). *Landed: `LauncherStatus.session_count: u8` + `session_profiles: Vec<SessionPip { color, initial }>` populated by `publish_session_snapshot`, which maps each session's unlocked-profile (if any) to `(color_hex, first_grapheme_uppercase(display_name))`. `ui/main_screen.rs::paint_player_orbit` is called *before* any widget laid out so the pips land above the vortex but below the heading/QR via egui's submission-order layering (no `LayerId` scaffolding needed). Ellipse scales as `rx = 29% × width`, `ry = 37% × height` — matches the mock's `560×400` against `1920×1080` proportions. Orbit speed `0.10 rad/s` lines up with the vortex's 0.08 rad/s ambient so the two slow rotations don't compete. Pip fill defaults to `GOLD_BRIGHT` when a session has no profile unlocked; initial defaults to a small dot.*
...
Discovered during a Mac-side browser smoke-test on 2026-04-17: the shipped Leptos SPA and the Phase 4 mocks have drifted in several visible ways. The original framing was "drag the code back to match the mock" — wrong lens. **Code behavior, pinned by tests, is the long-term truth; mocks are point-in-time design references that age the moment the design settles.** This pass evaluates each drift on its own merits and locks the outcome in with a test where coverage is missing. The mock HTML itself stays frozen as a reference photo — we do not re-sync it (reframe logged 2026-04-18).

Item tags:

- **[bug]** — shipped behavior is clearly wrong (missing component, a11y hole, defect). Fix + regression test.
- **[feature]** — missing capability regardless of mock. Implement + coverage.
- **[judgment]** — mock is one opinion, shipped code is another. Decide which we want, record the call, pin it. The mock is a prompt to think, not an order.
- **[verify]** — may already be done; check shipped behavior first, add a test if one's missing.

Ordered roughly by user-visible impact; group labels in bold separate areas of the app.
...
- [x] 4.18.1a ~~*[feature]* **mDNS/Bonjour advertisement for stable URL.**~~ **Done 2026-04-19** — `crates/server/src/mdns.rs` builds `http://<os-hostname>.local:<port>/#k=...` for the QR URL, falling back to the raw IP if the OS hostname can't be read. Took two iterations to get there: (1) `mdns-sd` (pure-Rust UDP/5353) registered cleanly but Windows ≥10 owns the responder and ignored our advertisements — iPhone resolution silently failed; (2) Windows native `DnsServiceRegister` registered the SRV/PTR/TXT service records but doesn't expose a way to publish a custom A record, so `<custom-host>.local` still didn't resolve (a covering `#[ignore]`-gated test pinned this). Final landing: don't push a custom hostname at all — Windows ≥10 v2004 auto-publishes `<computername>.local`, so we just read it via `GetComputerNameExW(ComputerNameDnsHostname)` and put it in the URL. Less branded (`http://chris-htpc.local:...` instead of `http://skylander-portal.local:...`) but it actually resolves on iOS — verified by Chris on real iPhone 2026-04-19. Diagnostic test `os_hostname_resolves_via_local` (gated `#[ignore]`) verifies the OS-published hostname IS reachable via getaddrinfo, so future regressions in the Windows mDNS responder get caught. **Closes 4.19.10b.**
- [ ] 4.18.1b *[feature]* **Loading / profile-picker: "Add to Home Screen" hint.** Depends on 4.18.1a. Render a one-time dismissible banner on first connect with the iOS install instructions (Share → Add to Home Screen). Persist dismissal in `localStorage`. Only show when UA matches iOS Safari and `navigator.standalone !== true` (already-installed PWA skips it). Android can be deferred — Chrome handles the prompt itself.
- [ ] 4.18.1c *[feature]* **Service worker for PWA cache + update detection.** Today the static-asset path returns `Cache-Control: no-store` on everything (4.15.11 commit) — fine for dev iteration but wasteful on cold starts and doesn't help iOS-PWA backgrounded-for-hours staleness. Add a service worker (`phone/assets/sw.js`, registered from `index.html` on first load) that: (a) caches hashed wasm/js/css/font assets immutably (long TTL, content-addressed), (b) treats `index.html` + `manifest.webmanifest` as `no-cache` (revalidate every load), (c) on activation, deletes stale cache entries from older bundles, and (d) optionally posts a "new version available" message to the running SPA so it can show a refresh banner. Once this lands, switch the server's static-fallback `Cache-Control` from `no-store` to per-path: hashed assets get `public, max-age=31536000, immutable`; `index.html` + manifest stay `no-cache`. Service worker is the only mechanism that survives iOS PWA's app-shell caching across long backgrounding — the header-only approach (current state) doesn't.
...
- [~] 4.18.5c **Revisit mock for menu-overlay → Konami-gate transition.** User flagged 2026-04-18: "transitioning from the overlay menu to the konami entry will need to be revisited in mock too since they are so different." Today the Leptos flow routes from `MenuOverlay` → (close + set manage_gate) → `ProfilePicker`'s `KonamiGate`, but the visual handoff is abrupt and the two surfaces have very different treatments.
    - [x] **Mocks done 2026-04-18.** `menu_overlay.html` gains MANAGE PROFILES as a single-tap action (gear `⚙`), a `gate-rise` transition (portal recedes + sinks 28px while gate heading/dots/pad cascade in from below), a `poststate-gate` placeholder that renders a full schematic gate using the same Titan One heraldic language as the menu panel (gold through-line), and a `.no-profile` demo toggle that collapses the menu to just MANAGE + SHUT DOWN + join-QR for the post-connect empty state. `profile_manage.html` gets a matching entry-cascade on its gate screen so the "landing-from-menu" reference renders identically whether arrived-via-transition or viewed standalone.
    - [ ] *[bug + judgment]* **Leptos re-land.** Bug portion: hide the empty `.menu-current-chip` row in `MenuOverlay` when no profile is unlocked (today the chip renders blank — clearly wrong regardless of mock). Judgment portion: decide whether the gate-rise + entry-cascade animations are worth the code weight vs a plain dismiss/mount; the mock's elaborate handoff is one opinion, a simple cross-fade is another. Whichever we pick, add a Leptos component test (or at minimum a `#[test]` asserting DOM class sequencing) so the chosen transition doesn't silently regress.

**Profile picker, profile create, profile manage** (`phone/src/screens/profile_picker.rs`).

- [ ] 4.18.6 *[judgment]* **`CreateProfileForm` pacing: single form vs 4-step wizard.** Current: one scrollable form. Mock's opinion: Name → Color → PIN → Confirm as four separate steps for kid-friendly pacing. Decide which we want given target users are 7-12yo; a single form may actually be fine if fields are clearly grouped. If we go wizard, lock step order + step-skip-guards with a component test.
- [ ] 4.18.7 *[feature]* **Prefilled random Skylander name + reroll button.** Missing regardless of mock — helps kids who can't type yet. No whitelist per 4.2.8. Lock with a test that `CreateProfileForm` mounts with a non-empty name and reroll produces a different value with high probability.
- [ ] 4.18.8 *[bug]* **PIN confirm step gives no mismatch feedback.** Current: mismatch silently fails or error is invisible. User can't tell why submission didn't work. Add a visible error state (shake + inline banner is one way) and pin with a test that confirms the error node renders on mismatch.
- [ ] 4.18.9 *[judgment]* **PIN reset: 1-step vs 2-step.** Current: 2-step (old PIN → new PIN). Mock's opinion: 1-step (new PIN only) because Konami gate already authenticated. Policy call: is the gate sufficient auth for PIN reset, or do we want defence-in-depth? Lock the decision with a test asserting the expected step count and failure modes.
- [ ] 4.18.10 *[feature]* **Profile "last used N days ago" / "never used" subtext.** Needs a per-profile last-used timestamp — derive from `MAX(figure_usage.last_used_at)` or add `profiles.last_used_at`. Lock with a DB integration test asserting the timestamp updates on unlock.
- [ ] 4.18.11 *[verify]* **Profile manage DEL uses HOLD TO DELETE, not `window.confirm()`.** 4.2.14's danger-hold pattern should be universal across destructive actions. Check shipped code; if still `confirm()`, wire to the `ResetConfirmModal` machinery. Pin with a test asserting the hold interaction is required.

**Game picker** (`phone/src/screens/game_picker.rs`).

- [ ] 4.18.12 *[feature]* **Per-card tagline + "currently playing" marker.** Content decision: hardcode a serial→tagline map or extend `InstalledGame`. Lock with a test asserting the active game carries the marker and every known serial resolves to a tagline.

**Portal + Browser** (`phone/src/screens/portal.rs`, `browser.rs`).

- [ ] 4.18.13 *[bug]* **"PORTAL" `<DisplayHeading>` missing above the slot grid.** Small visible gap vs intended layout. Add + a render test that asserts the heading mounts in the portal view.
- [ ] 4.18.14 *[feature]* **GAMES drill-down filter chip row in `BrowserHead`.** One chip per detected game. Lock with a browser filter-state test.
- [ ] 4.18.15 *[feature]* **CATEGORY drill-down filter row** (Vehicles / Traps / Minis / Items). Additive with elements, not exclusive. Confirm per-figure category metadata is on `Figure`; add if not. Lock with a combined-filter test.
- [ ] 4.18.16 *[verify]* **Toy-box lid grabber pill + swipe-hint copy.** 4.2.13.6's fix landed in the mock; check the Leptos `<Browser>` gesture zone. If missing, add + test the element presence; if present, add the test anyway so it doesn't regress.
- [ ] 4.18.17 *[bug]* **Ownership indicator per loaded slot missing.** Required by CLAUDE.md for 2-player sessions (Phase 3 deferral 3.10.7). Profile swatch overlay on the bezel showing who placed the figure. Lock with a multi-session integration test asserting slot events carry owner identity.

**Figure detail** (`phone/src/screens/figure_detail.rs`).

- [ ] 4.18.18 *[bug]* **Action buttons are glyph-only (a11y hole).** Touch users never see `title=` tooltips, so ✦ ☰ ↺ are unlabelled in practice. Add visible text labels ("APPEARANCE" / "STATS" / "RESET"). Lock with a test asserting each button has accessible text.
- [ ] 4.18.19 *[verify]* **Hero-aura + hero-rays paint behind lifted figure bezel.** `<FigureHero>` owns this per 4.4.5 — check whether the visual shipped or only the layout. If shipped, add a CSS-class-presence test; if not, implement + test.
- [ ] 4.18.20 *[judgment]* **Ghost-grid / box-backdrop context on figure detail.** Current: `lib.rs:203-218` replaces portal+browser with detail via `<Show fallback>` — underlying context disappears. Mock's opinion: keep them at ~25% opacity behind the panel (per 4.3.7) for spatial continuity. Replacement is simpler and arguably cleaner; overlay is more immersive. Decide, then pin the chosen screen-stack behavior with a test.

**Modals & overlays** (`phone/src/screens/modals.rs`, `phone/src/lib.rs`).

- [x] 4.18.21 ~~*[bug]* **ConnectionLost overlay is a TODO stub.**~~ **Done 2026-04-19** — `phone/src/screens/connection_lost.rs` shipped: pulsing red ✕ pip, "LOST CONNECTION" heading, reconnect spinner, manual "TRY AGAIN" button after `RETRY_THRESHOLD = 3` failed auto-retries, Wi-Fi hint. Mounted at the bottom of `App()`'s view (z=90 — above GameCrashed at 70) so it preempts the stack without restructuring the route flow. 1s grace via `spawn_local` re-check absorbs momentary drops. `ws.rs` extended with `reconnect_attempts: RwSignal<u32>` (incremented on each onclose, reset on successful onopen) and `manual_retry: RwSignal<u32>` (UI bumps; an Effect cancels the pending backoff timer and reconnects immediately). **Persistence fix 2026-04-19**: first cut watched `conn == Disconnected` and flashed off every backoff tick when conn flipped to `Connecting`; rewritten to drive off `reconnect_attempts > 0` (only resets on successful onopen) so the overlay stays visible through the entire reconnect cycle. Verified on-device by Chris. E2E regression `crates/e2e-tests/tests/connection_lost.rs` exercises kill-server → overlay-appears AND a 18s persistence assertion (covers the regression that motivated the rewrite); console output is mirrored into `window.__capturedLogs` and dumped on failure for one-shot diagnostics. Phone→server log-forwarder (`POST /api/_dev/log` + `phone/src/dev_log.rs` + `dev_log!` / `dev_warn!` macros) ships disconnect-period traces to the launcher's PowerShell so on-device debugging doesn't need a Mac + Web Inspector — buffer retains entries on POST failure (7 unit tests) and the server handler is unit-tested (6 oneshot tests). Reconnect→overlay-vanishes path is covered by manual on-device smoke testing for now (harness can't bring a dead server back on the same port).
- [ ] 4.18.22 *[feature]* **`ResumeModal` element-tinted bezel plates.** Currently uses a generic `<GoldBezel>`. Add `.plate.{element}` tinting so the preview reads as magic/fire/life/tech at a glance. Lock with a render test asserting the tinted class lands on each bezel based on figure element.
- [ ] 4.18.23 *[feature]* **`ResumeModal` relative-time subtext.** Replace generic "saved layout" with "saved today" / "saved N days ago". Needs `saved_at` on `ResumeOffer` (server-side `profile_layout.saved_at` or equivalent). Lock with a test that renders known relative-time strings for sample timestamps.
- [ ] 4.18.24 *[judgment]* **`MenuOverlay` post-action transitions.** Current: plain dismiss. Mock's opinion (per 4.2.14.a): bespoke `identity-drain` / `fold-away` / `lights-dim` per action. Decide whether the weight of three custom transitions is worth the narrative reinforcement vs a shared clean exit. Whichever we land, lock DOM-class sequencing with a test.

**Wrap-up.**

- [ ] 4.18.25 Re-run the browser smoke-test on a real iOS device once 4.18.1 ships; walk each screen against the agreed behavior from this section (not the mock — the mock is aging too) and add any missed drift items before closing this pass.
...
- [x] 4.19.2a ~~*[feature]* **No "Startup" surface.**~~ **Done 2026-04-19** (visuals refined through screen-by-screen review the same day). `crates/server/src/ui/launch_phase.rs` introduces `LaunchPhase::{Startup,Transitioning,AwaitingConnect}` derived from `time_since_mount` + activity. Startup paints the full sky backdrop + a bold "STARTING" Titan-One title (embossed body with stacked carve under-layers + a soft two-ring gold ellipse glow via `paint_heraldic_title`, sized at 140px to match mock presence); Transitioning ramps `iris_radius` 0→1.2 over 0.6s while the QR/status snap in; AwaitingConnect parks at the steady main render. Activity (rpcs3_running OR clients>0) short-circuits the reveal. 8 unit tests cover the phase math + monotonic iris + content-visibility contracts.
    
    **Reusable infrastructure that came along for the ride** (now available to other 4.19 items):
    - `vortex::paint_sky_background` — 3-layer launcher backdrop applied behind every screen: vertical SF_1 → SF_2 → SF_3 gradient base (mid-stop at 85% so most of the panel stays "deep ocean"), top SF_1-tinted ellipse, bottom SF_2-tinted ellipse extended past panel corners. Replaces the prior flat-SF_3 panel fill.
    - `vortex::paint_starfield` — ~36 sparse procedural stars with deterministic angular bearing + slow radial-outward drift (5px/s, fade-in near centre + fade-out near edge to hide the wrap). Doesn't conflict with the vortex's rotation since the vectors are orthogonal.
    - `vortex::paint_radial_ellipse` (now `pub`) — general-purpose triangle-fan radial gradient. Used by the sky background, the title glow, and ready for 4.19.7 (focal halo behind QR) + 4.19.18 (compile-progress ring backdrop).
    - `vortex::paint_vertical_gradient` — 6-vertex 3-stop linear gradient mesh.
    - `paint_heraldic_title` — Titan-One title with embossed shadow stack + soft ellipse glow. Direct `Painter::text` calls (not `ui.heading`), so reusable from any layout context.
    - `palette::apply` now pins `ThemePreference::Dark` first, fixing a Windows-11-light-mode bug where Visuals got clobbered back to cream after our set_visuals call.
...
- [ ] 4.19.5 *[judgment, downgraded from feature 2026-04-19]* **Vortex shader: cheap polar-mesh sin/cos vs spec'd 5-octave simplex FBM.** Was promoted to `[feature]` after the "shaders look horrible" feedback, but the surrounding visuals improvements that landed under 4.19.2a (proper sky-gradient backdrop, top/bottom hue ellipses, drifting starfield, halo-glow primitive) carry a lot more of the visual weight than the cheap vortex did on its own. **Re-eval before committing to the egui_wgpu port** — the vortex may now read fine in context, in which case the cheap polar-mesh stays and this stops blocking other 4.19 work.
...
- [ ] 4.19.7 *[verify]* **Halo focal-glow missing.** Spec §3.3 + §3.6: 760px-equivalent radial gradient with `mix-blend-mode: screen` behind the QR / progress ring. Code paints the QR/heading without an underlying glow layer. Confirm in-app on the TV — without the halo the QR may read as a flat sticker on top of the clouds rather than emerging from them. **Primitive ready 2026-04-19**: `vortex::paint_radial_ellipse` (now `pub`) is the same gradient-via-mesh helper used for the title glow + sky background; just call it from `render_main` between the vortex draw and the QR card-flip with the QR's centre + ~half-panel-width radii.
...
- [ ] 4.19.10a *[bug]* **URL string rendered on screen (user-flagged 2026-04-19).** `main_screen.rs:88` paints the raw `self.url` as `RichText`; spec §3.3 explicitly says "**No URL text shown** — just the QR + 'SCAN TO CONNECT' in Titan One." Drop the URL render. The QR itself carries the URL — phones don't read it off the screen. Lock with a render test asserting no `\u{0068}ttp` substring is in the launcher viewport.
- [x] 4.19.10b ~~*[feature]* **QR URL should be mDNS-based.**~~ **Done 2026-04-19** — closed alongside 4.18.1a; the QR now encodes `http://<os-hostname>.local:8765/#k=...` and survives DHCP changes. See 4.18.1a for the why-not-a-custom-hostname rationale.
...
- [ ] 4.19.22 *[bug]* **Awaiting Connect surface carries debug-info noise the mock omits (user-flagged 2026-04-19).** Side-by-side with `tv_launcher_v3.html` state 4: mock shows ONLY the QR + a big heraldic "SCAN TO CONNECT" label below it. Today `render_main` adds a top-of-panel `SKYLANDER PORTAL` heading, a status strip ("no game running" — dead context, since render_main only ever shows when no game IS running), the literal URL string under the QR (covered separately by 4.19.10a), "Waiting for phone…" / "N devices connected" status text, and "504 figures indexed" debug counter. Drop all five — the orbit pips already convey "phone joined", and the brand identity is established by the launcher icon + window title. Keep the **Exit to Desktop button** (not in mock — implementation pragmatism so the user isn't trapped if shutdown via phone fails). Lock with a render test asserting only QR + heraldic label + button are present.
- [ ] 4.19.23 *[feature]* **Server-error launcher state for "server failed to start" (user-flagged 2026-04-19).** Today if HTTP bind fails, DB open fails, or wizard config is invalid, the eframe launcher still paints but the QR URL is dead silent — user has no signal beyond "phone scan does nothing." Add a `LauncherScreen::ServerError { message }` variant; main.rs's tokio thread catches startup errors and posts them via `LauncherStatus.screen` instead of panicking. Render builds on the Max Players card-flip back-face design (same gold bezel, same red-tint plate, "SOMETHING'S NOT RIGHT" heading + the diagnostic message + "RESTART"/"EXIT TO DESKTOP" gold buttons). Soft error pattern — recoverable feel, not crash-screen feel.
...
- [ ] 4.19.18 *[feature]* **Progress ring + "COMPILING SHADERS" heading missing (depends on 4.19.4 + 4.19.17).** Even with detection wired, the visual is a follow-up: gold conic-gradient ring (200–240px), `current / total` count in centre, ring-flash on completion. Skip if 4.19.17 lands without log-derived progress data. **Building blocks ready 2026-04-19**: `paint_heraldic_title` for the heading + subtitle, `paint_radial_ellipse` for the ring backdrop glow.

**§3.7 Typography.**

- [ ] 4.19.19 *[verify, half-done]* **Hero size 80px in code vs 96px in spec.** Plus the farewell-heading 72px-vs-64px deviation flagged in 4.19.16. Walk the type scale on the actual TV and either bring code in line or update the spec. One pass, then pin sizes per surface with a test. **Half-done 2026-04-19**: Startup's "STARTING" title now uses `paint_heraldic_title` at 140px with the embossed shadow stack + ellipse glow (Chris's screenshot review showed 96px was way too small — mock fills ~50% of panel width). Still flat-and-small for `render_main`'s "SKYLANDER PORTAL" steady-state title (`ui.heading(...)` at 80px, no embossing) and the crash/farewell screens. Switching them over requires extracting from the layout-stack `vertical_centered` flow into direct painter calls — same pattern as render_brand_intro.
