...
## Phase 4 - Phone UI drift reconciliation (residuals)
...
- [x] 10.8.5 - **Steam Big Picture: missing icon + banner artwork. DONE (2026-05-30).**
  Two parts: (a) **Start Menu / Desktop shortcut icons** — the MSI's advertised
  `<Shortcut>`s lacked an explicit `Icon=`, so they rendered blank despite the
  exe embedding `icon.ico`; fixed by referencing the shared `<Icon>` (Id given a
  `.ico` suffix) with `IconIndex='0'`. (b) **Steam library artwork** — a first-pass
  set (`library_600x900` grid, `library_hero_1920x620`, `logo.png`,
  `library_header_920x430`, `icon_256`) generated by `tools/steam-art/generate.py`
  (Pillow, starfield/gold aesthetic from `icon.ico` + TitanOne), committed in
  `steam/`, shipped in both the MSI (heat now harvests the populated `steam/`) and
  the zip. Steam can't auto-apply non-Steam artwork, so the README documents the
  one-time "right-click → Manage → Set Custom Artwork" step. (An auto-installer
  that writes into Steam's per-user `grid/` folder by computed appid was considered
  and deferred — fragile + can clobber user art.)
...
- [x] 20.3 - **Windowed `native_options` in Desktop mode** *(done 2026-06-09, folded in with 20.2)*. `main.rs` viewport: `windowed = spike || cfg.window_mode == Desktop` → resizable window, `min_inner_size 480×360`, decorated (default), not always-on-top; TV stays fullscreen. Transparency kept in both real modes (only the 20.1 spike is opaque) for the future in-game iris overlay. Original task text: (`main.rs`, currently
  ...
  decorated for v1.
- [x] 20.4 - **Extend window coordination to track launcher geometry (the resize work).** *(done + live-validated 2026-06-09 on the HTPC — Spyro's Adventure tracked launcher move AND resize)* — `fit_game_below_launcher_via_win32` (`ui/mod.rs`), gated on `LauncherApp.window_mode == Desktop`: moves+resizes the game to the launcher's client rect **every frame**. (Diagnosis: RPCS3 comes up fullscreen and re-sizes its own window during boot, so a change-detected skip let it drift out of the fit — always-apply fixes it; SetWindowPos to identical geometry is a near-no-op. Guards minimised/degenerate rects.) Also gated the Main-screen action row on `!game_in_progress` so the buttons don't sit on top of the live game (the dispatcher still calls `render_main` while `game_underneath`; badge+label fade via alpha, the egui buttons didn't). `window_mode` threaded into `LauncherApp::new`. In
