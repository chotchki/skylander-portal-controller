...
- [~] 6.5 **NFC-scan import via ACS122U reader.** Plus-button in the toy-box view; one-way import (read-only, no write-back per 2026-04-22 scope call). Scanned figures join the shared toy-box library — available to any profile, working copies kept per-profile (same semantics as pack-derived figures in CLAUDE.md). Primary goals: (a) discourage piracy by making scan-and-own the easiest onboarding path; (b) decouple figure discovery from user-maintained file structure; (c) accommodate users who have physical figures but no existing .sky backup. Child items gate on 6.5.0's spike outcome.
  ...
  - [ ] 6.5.5 **Indexer refactor: parse-driven identity** (supersedes 6.2.7). With scanning as a live figure source, folder-walk discovery is vestigial. Indexer reads `(figure_id, variant)` from block 0 of every .sky regardless of origin (pack or scanned tree). `data/figures.json` rekeyed from SHA-based `FigureId` to the tag-ID scheme; `tools/wiki-scrape` regenerated against the new key. Biggest blast-radius item in the 6.5 group — touches `crates/indexer/`, `crates/core/src/figure.rs`, all `crates/server/*` consumers, `tools/wiki-scrape`, `crates/indexer/tests/real_pack.rs`. Do after 6.5.1–6.5.3 so the migration has a second source of truth (scanned figures) to validate against.
