...
- [x] V.7 - Publish the patched binaries at the new pin
  ...
  - [x] V.7.2 - Gated full build DONE - `rpcs3-patched-accfecd2d` prerelease carries all 4 assets: `rpcs3-patched-windows-x64.zip` (59.3 MB) + `.sha256`, `rpcs3-patched-macos-arm64.tar.gz` (66.3 MB) + `.sha256`. Took 3 dispatches: `35536851380` (Windows died at apply -> V.11; cancelled mid-macOS by a push), `35537953773` (macOS GREEN + published; Windows died on stale LLVM pin -> V.12), `35539244118` (Windows-only via the new `targets` input -> GREEN). `release.yml`'s download lanes are unblocked. -> publishes the `rpcs3-patched-accfecd2d` prerelease with the Windows zip + macOS arm64 tarball. **BLOCKS the next release tag** - `release.yml`'s two download steps already point at that tag and will fail until it exists.
...
- [x] V.11 - Fix the Windows `apply.sh` regression from V.5 (gated build run `35536851380` died at "Apply patch series"). My gitlink-safety fix staged only files `git diff` called changed - but under Windows `autocrlf=true` the clean filter strips CRLF on the way to the index, so a file just rewritten CRLF->LF reads as UNCHANGED while its stat entry is dirty, and `git am` aborts with "Your local changes would be overwritten by merge". Now `git add`s every series-touched file (re-stats them, clearing the dirt) and commits only if the index actually moved - both constraints held. Reproduced + verified locally under a `core.autocrlf=true` clone, so this is no longer a CI-only blind spot.
- [x] V.12 - Fix the Windows full build's STALE dependency pins (run `35537953773`: apply step green - V.11 held - then MSVC died in `JITLLVM.cpp`, pure upstream code, on a `setTargetTriple(llvm::Triple)` overload that only exists in LLVM 22). `rpcs3-patched.yml`'s build-windows env is a hardcoded copy of upstream's `Windows_Build` env and had rotted: `LLVM_VER` 19.1.7 -> **22.1.8**, `QT_VER` 6.11.1 -> 6.11.2, `QT_DATE` likewise. Synced, plus a **guard step** that diffs our block against upstream's at the pinned commit and fails in SECONDS instead of 30 minutes (tested both ways locally against the real pinned file). Added to the README rebase checklist as step 3b.
- [x] V.13 - Add a `targets` dispatch input (`both`/`windows`/`macos`) to `rpcs3-patched.yml`. A one-leg failure was forcing a full re-dispatch, rebuilding the leg that already passed - that is how this phase burned two macOS hours. macOS minutes are the scarce budget (`docs/dev/ci.md`).
- [ ] V.14 - Cut `v1.10.0` as a Windows-testable BETA (first release carrying the new pin)
  - [x] V.14.1 - Tag `v1.10.0` on green `main` + push. Minor bump, not a patch - the bundled emulator moved 516 commits under the user. NOTE the tag must stay NUMERIC: `cargo wix --install-version` gets it with the `v` stripped and WiX rejects a semver prerelease suffix, so `v1.10.0-beta.1` would break the MSI step.
  - [x] V.14.2 - Watch `release.yml` - run `35543190949` GREEN on BOTH lanes, first try. 4 assets: Windows zip (117 MB) + MSI (103 MB), macOS dmg (104 MB) + tar.gz (25 MB). The bundled-RPCS3 download step resolved `rpcs3-patched-accfecd2d` cleanly, so the V.3 tag bump was right.
  - [x] V.14.3 - `gh release edit v1.10.0 --prerelease --latest=false` once the run is fully done - DONE, verified: `/releases/latest` still resolves to v1.9.21. (`gh release list` still PRINTS "Latest" beside v1.10.0 - that column is newest-by-date, not the latest pointer; trust the API.) - both lanes hardcode `prerelease: false`, so flip it AFTER publish or the second lane resets it. Keeps v1.9.21 as Latest while the pin is unvalidated on hardware.
  - [ ] V.14.4 - After V.8 passes on real hardware, promote: `gh release edit v1.10.0 --prerelease=false --latest`.
- [ ] V.8 - Live-validate on real hardware (chotchki): boot Giants on the new binary - IPC listener up, LOAD/CLEAR, P8 surface composite. Artifact is the **v1.10.0 prerelease** (Windows zip or MSI; the zip avoids installer state). NOTHING has booted a game on this emulator anywhere yet - the local build only proved it compiles and the P1 IPC strings are present. On failure suspect the two hand-resolved hunks first: P5's `sys_usbd` reconnect path and P1's `load_skylander` call site.
