...
- [ ] A.8 - Video 2 (Tour) — comprehensive feature walkthrough (`walkthrough`, IpcCold / real RPCS3)
  ...
  - [ ] A.8.9 - `ownership` pips — inject a 2nd profile (Bob); a headless non-captured phone binds via set_session_profile + places via REAL IPC so the captured phone shows per-slot ownership pips. Extend BeatCtx (bob id + chromedriver url)
  - [ ] A.8.10 - Mac product gap (chotchki) — easy first-launch wizard re-trigger: a --reconfigure flag that runs the wizard ignoring existing config (today you hand-delete config.json); doc it. Serves the `install` beat too
  - [ ] A.8.11 - `install` beat — boot via --reconfigure into the egui wizard, capture it as a standalone clip, splice into the Tour at render (new render-concat capability)
  - [ ] A.8.12 - `farewell` beat — graceful-shutdown egui Farewell surface as the in-narrative closer (trigger shutdown + hold the launcher pane; no phone drive)
