...
- [x] 4.18.21 ~~*[bug]* **ConnectionLost overlay is a TODO stub.**~~ **Done 2026-04-18** — `phone/src/screens/connection_lost.rs` shipped: pulsing red ✕ pip, "LOST CONNECTION" heading, reconnect spinner, manual "TRY AGAIN" button after `RETRY_THRESHOLD = 3` failed auto-retries, Wi-Fi hint. Mounted at the bottom of `App()`'s view (z=90 — above GameCrashed at 70) so it preempts the stack without restructuring the route flow. 1s grace via `spawn_local` re-check absorbs momentary drops. `ws.rs` extended with `reconnect_attempts: RwSignal<u32>` (incremented on each onclose, reset on successful onopen) and `manual_retry: RwSignal<u32>` (UI bumps; an Effect cancels the pending backoff timer and reconnects immediately). E2E regression `crates/e2e-tests/tests/connection_lost.rs` exercises kill-server → overlay-appears with `TestServer::kill_server()`. Reconnect→overlay-vanishes path is covered by manual on-device smoke testing for now (harness can't bring a dead server back on the same port).
