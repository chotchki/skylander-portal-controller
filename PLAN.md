...
- [ ] 4.15.15 **Research spike: egui shell as RPCS3 cover + input routing.** The shell needs to stay on top of RPCS3 so the user never sees Qt's transient noise — `#32770` file pickers we already sling off-screen, plus the once-per-session Qt menu popups that clamp to upper-left during `open_dialog` / `quit_via_file_menu` (PLAN 3.6b known residual). Chris noted during 3.7 debugging that Windows also generates display-mode flashes on RPCS3 boot that are jarring on the 86" TV. Keeping egui fullscreen + `WS_EX_TOPMOST` covers all of this *visually*, but breaks our automation: `boot_game_by_serial` currently uses `SendInput` mouse clicks at screen coordinates, which land on whichever window is topmost at that position — egui, not RPCS3. Three options to evaluate: (a) `WS_EX_TRANSPARENT` click-through on egui (lets SendInput pass through but means egui can't receive any mouse of its own — fine if phone-only remote works); (b) temporarily lower egui Z-order during clicks, restore after (straightforward, but creates a brief visible gap during each boot); (c) switch from SendInput to `PostMessage(WM_LBUTTONDOWN/UP)` targeted at the RPCS3 library HWND with window-relative coords — bypasses Z-order entirely and means egui can stay topmost + interactive. (c) is the cleanest long-term answer; validate it still works when the target window is partially/fully covered. Also validate: does RPCS3's viewport fight egui for topmost when a game is running? Does forcing RPCS3 into windowed mode (not fullscreen) eliminate the display-mode flashes? Deliverable: a small probe under `crates/rpcs3-control/examples/` and a recommendation written into 4.15.8 (in-game transparency) + 4.15.9 (game-switching transition).
