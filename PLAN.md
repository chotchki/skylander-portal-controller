...
- [ ] V.14 - Cut `v1.10.0` as a Windows-testable BETA (first release carrying the new pin)
  - [ ] V.14.1 - Tag `v1.10.0` on green `main` + push. Minor bump, not a patch - the bundled emulator moved 516 commits under the user. NOTE the tag must stay NUMERIC: `cargo wix --install-version` gets it with the `v` stripped and WiX rejects a semver prerelease suffix, so `v1.10.0-beta.1` would break the MSI step.
  - [ ] V.14.2 - Watch `release.yml`: Windows lane (zip + MSI, pulls the bundled `rpcs3-patched-accfecd2d` zip) and the env-gated macOS lane (signed/notarized dmg).
  - [ ] V.14.3 - `gh release edit v1.10.0 --prerelease --latest=false` once the run is fully done - both lanes hardcode `prerelease: false`, so flip it AFTER publish or the second lane resets it. Keeps v1.9.21 as Latest while the pin is unvalidated on hardware.
