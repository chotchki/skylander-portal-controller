...
- [x] 4.2.14 **Destructive confirmation — hold-to-activate pattern.** User-review follow-up: RESET (figure) and SHUT DOWN (server) are single-tap today; adding a ~1.2s press-and-hold with a white progress fill sweeping left→right so a fat-finger tap can't wipe a figure or kill the app. Release-to-cancel; complete-to-fire (button flashes). Applied to `reset_confirm.html` (RESET → HOLD TO RESET) and `menu_overlay.html` (SHUT DOWN → HOLD TO SHUT DOWN). `design_language.md` §4 gains the "Destructive confirmation — hold-to-activate" subsection plus a contrast note that **recoverable** destructive actions (REMOVE) stay single-tap.
    - [ ] 4.2.14.a **Post-destructive-action state animations (deferred to state-animation pass).** After the hold completes, mocks currently just flash white and log. Real flow should play a themed transition before the next screen/state resolves. Design in-situ with 4.2.11 / 4.5 ambient motion so they share token vocabulary. Targets:
        - **RESET (figure)** — *local* to the acting phone. Gold flakes fall off the bezel, hero plate desaturates / returns to a fresh-figure look, panel fades back to the figure detail view showing the reset figure.
        - **CHOOSE ANOTHER GAME** — affects **every connected phone**. Trigger is the server's existing game-exit broadcast, NOT a local button handler. On each session the portal "folds away" into the game picker. Don't wire the animation to the button click; wire it to the WS event.
        - **SHUT DOWN** — affects **every connected phone**. Trigger is the server's shutdown event + WS close. Each phone plays a "lights out" farewell (starfield + bezels dim, gold lettering recedes) before the socket closes the tab. Locally the button's `.fired` flash is the lead-in; the broadcast is what drives the rest.
        - **SWITCH PROFILE** — local only; standard profile-re-lock transition, no broadcast needed.
