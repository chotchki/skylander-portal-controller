...
- [x] 4.15.12 **Shader-compile detection.** Closed 2026-04-23 (already shipped). `spawn_shader_compile_watchdog` in `state.rs:428` tails RPCS3's log file; `classify_log_line` buckets lines into "Building SPU cache" / "Building PPU cache" / "Compiling shaders" with a lowercase-first tolerance pass; `shader_compile_text` publishes the current phase; `game_playable` flips true once log activity's been quiet for ~2s. Unit tests cover every bucket + the empty/no-match path.
- [x] 4.15.13 **Shader-compile progress visualization.** Closed 2026-04-23 (already shipped). QR card flips to a "LOADING" badge via `qr_card_flip` + `paint_titled_card`; `paint_loading_halos` sweeps two concentric gold conic-gradient arcs (slow outer + fast inner, mock's `.state-loading` look). Subtitle swaps to the booting game's name during load; a dim italic phase line ("Compiling shaders", etc.) appears below. Deferred polish (may revisit): numeric shader count ("42/128" — needs RPCS3 log-message parsing) and a gold completion flash when `game_playable` flips true. Neither is painful in real use so neither's on the MVP path.
- [x] 4.18.27 *[bug → feature]* **Profile-create → staged wizard.** 2026-04-23. `CreateProfileForm` refactored from one long form to a 4-step wizard: Name → Color → Pin → Confirm. Each step owns its sub-body + shares a "Step N of 4" chip + Titan One title + BACK/NEXT footer, so the iPhone viewport only needs to fit one step at a time (previously the confirm keypad scrolled below the fold). NEXT validation is per-step (Name requires non-empty; Pin requires 4 digits; Confirm validates via the existing submit path and lands the error banner + shake on mismatch). BACK on Step 1 cancels out of the flow (preserves the old CANCEL behaviour). Scroll-fix `.screen-profile-picker { overflow-y: auto }` stays in as defensive fallback.
- [x] 4.18.28 *[bug]* **Profile-create color-scheme alignment.** 2026-04-23. Landed alongside 4.18.27. Retired the legacy `.create-profile-form, .pin-entry, .profile-admin { background: #161616; border: 1px solid #333; … }` rule that was painting a gray container inside the heraldic FramedPanel. New `.create-profile-wizard` + `.create-step-chip` (gold pill with Fraunces italic "STEP N OF 4") + `.create-step-title` (Titan One gold with embossed text-shadow) + `.create-step-body` establish wizard chrome that sits naturally inside the gold-framed panel. Buttons were already heraldic via `.btn-primary` / `.btn-cancel`; PIN keypad already heraldic via 4.18.6a.
- [-] 4.18.6 *[judgment]* CreateProfileForm pacing — overcome by events (2026-04-23). The pacing question was resolved in favour of the staged wizard when 4.18.27 shipped; single-form variant is retired.
- [x] 4.19.14 *[bug]* **Farewell breathe pulse.** 2026-04-23. Per spec §3.5, the farewell badge breathes during the 3s countdown: 2.4s sin wave, scale ±2.5% (both axes), opacity half-rectified to dip to 0.975 at phase=-1 and rise to 1.0 at phase=+1 (never overshoots full opacity). Freezes the instant the fade-to-black (4.19.15) starts so the engulfing black doesn't fight a moving badge. Horizontal axis carries the intro coin-flip + breathe combined via `intro.badge_scale() * breathe_scale`; vertical uses `breathe_scale` only since the coin-flip is horizontal-only. Repaint cadence tightened to 60fps throughout farewell (previously 500ms during countdown) so the sine wave renders smoothly — free compute since the vortex backdrop was already painting at 60fps.
- [x] 4.19.15 *[feature]* **Farewell fade-to-black before exit.** 2026-04-23. Per Chris's refinement of the PLAN item: after the 3s countdown, paint a full-panel black rect with alpha ramping 0 → 1 via ease-in cubic over 800ms, then fire `ViewportCommand::Close`. Overlay is painted last in `farewell::render` so it dims the badge + countdown + vortex backdrop together. Repaint cadence tightens to 60fps during the fade window (up from 500ms during the countdown read beat) so the alpha ramp is smooth. `FAREWELL_FADE` const kept alongside `FAREWELL_COUNTDOWN` so total duration reads as one number at a glance.
- [x] 4.19.12 *[bug]* **Reconnect QR fade-in.** 2026-04-23. New `reconnect_qr_shown_at: Option<Instant>` on LauncherApp, stamped when `clients` transitions to 0 while rendering the in-game surface and cleared when clients > 0 OR when the launcher leaves the in-game surface (so back-to-back sessions always start fresh). Passed into `in_game::render` as `reconnect_fade_elapsed_s`; the function computes `ease_out_cubic(elapsed / 1.0s)` and scales every paint call's alpha (gold ring, screen rim, QR texture tint, title + subtitle) through a `with_alpha` helper. The 60fps repaint cadence set at the top of the dispatcher makes the animation smooth.

**4.19 overlap sweep (closed 2026-04-23):** Items duplicated by already-shipped work from the 4.15 launcher line, plus one copy-string drift fix.
- [x] 4.19.3 *[feature]* Switching Games surface — shipped as 4.15.9 (iris-closed DarkHole + "SWITCHING GAMES" heraldic heading; `render_switching_heading` in `main_screen.rs`).
- [x] 4.19.4 *[feature]* Compiling Shaders surface — shipped as 4.15.12 (detection) + 4.15.13 (viz). QR card flips to LOADING badge with rotating halos + shader_compile_text subline.
- [-] 4.19.5 *[judgment]* Vortex shader approximation — closed alongside 4.15a.7 (Chris happy with current GLSL render; wgpu migration parked).
- [x] 4.19.9 *[bug]* Max-players copy — 2026-04-23. Changed `BackFace::MaxPlayers` lines from `["MAXIMUM", "PLAYERS", "REACHED"]` → `["PORTAL", "IS", "FULL"]` per `navigation.md` §3.3 spec.
- [x] 4.19.10a *[bug]* URL string rendered on screen — verified: `url_for_ui` in `main.rs` flows only to `main_screen::render_qr_texture`; no `RichText` / `ui.label` for the URL exists today. Removed alongside 4.19.22's debug-strip cleanup.
- [x] 4.19.17 / 4.19.18 *[feature]* Shader-compile detection + progress — duplicates of 4.15.12 / 4.15.13. Both shipped.
- [x] 4.19.22 *[bug]* AwaitingConnect debug noise — verified: `render_main` in `main_screen.rs` paints only the orbit pips, QR card, SCAN TO CONNECT subtitle, optional shader_compile_text, and Exit-to-Desktop button. Brand heading + status strip + URL text + client count + figures-indexed debug all absent.

**Verify sweep (4.18 + 4.19, closed 2026-04-23):**
- [x] 4.18.11 *[verify]* Profile DEL uses HOLD TO DELETE. Confirmed: `profile_picker.rs:454` triggers `deleting.set(true)` → "HOLD TO DELETE {name}" banner. No `window.confirm()` in UI code.
- [x] 4.18.16 *[verify]* Toy-box lid grabber pill shipped. `.lid-grabber-p4` is a 46×5px wooden pill in `components/toy_box.rs` + `app.css`. Text swipe-hint deliberately not shipped — the pill shape is a conventional drag handle; added copy would be noise.
- [x] 4.18.19 *[verify]* Hero-aura + hero-rays shipped. `components/figure_hero.rs:44-46` wraps the bezel with `.hero-aura` + `<RayHalo>`.
- [-] 4.19.7 *[verify]* Halo focal-glow behind QR — not shipped, deferred as polish. Steady-state QR reads fine at 10ft; `paint_loading_halos` already covers the Loading phase. Revisit if the steady-state QR reads flat on-TV.
- [x] 4.19.8 *[verify]* Pip orbit speed — real drift, fixed 2026-04-23. `ORBIT_SPEED` constant was 0.10 while the code comment + `navigation.md` §3.3 spec said 0.08. Pulled to 0.08.
- [x] 4.19.10 *[verify]* SCAN TO CONNECT label — code-is-truth. Code renders at HEADING_LG (64px) below the card; mock's 36px above is 1080p-era sizing that doesn't survive 86" @ 10ft. Current sizing locked in.
- [x] 4.19.11 *[verify]* QR bezel size — code-is-truth. `CARD_SIZE = 420px` (not 320 as the PLAN item said) on 4K TV reads at ~5.5% of screen width; mock's ~280px would be too small. Confirmed per the in-PLAN note about 4K headroom.
- [x] 4.19.13 *[verify]* Reconnect QR copy — already "scan to rejoin" per `in_game.rs:138`. Inset + size not measured precisely but the reconnect surface reads at real viewing distance on-TV.
- [x] 4.19.16 *[verify]* Farewell heading size — doesn't apply as written. `paint_titled_card` auto-scales the three-line title to the inscribed square of the CARD_SIZE badge; there's no fixed 72 or 64 constant to compare against.
- [x] 4.19.19 *[verify]* Hero title size — superseded by 4.19.22. The Main screen's fixed-size hero title was stripped when 4.19.22 dropped the debug-info overlay; only the Startup "STARTING" brand intro remains at HERO_INTRO=140px (embossed via `paint_heraldic_title`). Farewell auto-scales as noted above. New `Effect` in `phone/src/lib.rs` tracks `current_game.map(|g| g.serial)` and, on any transition after the very first Effect run, clears `search` / `element_filter` / `game_filter` / `category_filter`. The first-run skip uses a `Cell<bool>` so booting into an already-running game (fetch_status returns Some on mount) doesn't clobber signal initial values the user never touched. Serial-based compare means re-picking the same game is a no-op.
