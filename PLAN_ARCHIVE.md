...
- [x] 4.18.29 *[bug]* **Reset browser filters on game change.** 2026-04-23. New `Effect` in `phone/src/lib.rs` tracks `current_game.map(|g| g.serial)` and, on any transition after the very first Effect run, clears `search` / `element_filter` / `game_filter` / `category_filter`. The first-run skip uses a `Cell<bool>` so booting into an already-running game (fetch_status returns Some on mount) doesn't clobber signal initial values the user never touched. Serial-based compare means re-picking the same game is a no-op.
