...
**Latest release:** [v1.7.0](https://github.com/chotchki/skylander-portal-controller/releases/tag/v1.7.0) — **portal control moved from GUI automation to a patched-upstream RPCS3 over a local IPC socket (Phase 16).** Instead of pantomiming a human in RPCS3's Skylanders Manager dialog, the controller now drives the emulated portal directly over an AF_UNIX socket on a thin, additive patch series (vendored + GPL-compliant), and reads liveness/readiness from the emulator's own heartbeat — no dialog-driving, no window-title scraping, no boot/reveal flicker, and the game boots `--no-gui` straight into play. The **Windows release now bundles the patched RPCS3**; the first-launch wizard just points at your *existing* RPCS3 install for firmware + games. New on the launcher: an **"Open in Browser"** fallback for when a phone can't scan the QR (drive the portal from the HTPC itself). Figure dumps are now recognized in `.dump` / `.dmp` / `.bin` as well as `.sky` (same 1024-byte format). The repo also relicensed **MIT → GPL-2.0-only** (RPCS3 is GPLv2; the IPC patches are a derivative work).
